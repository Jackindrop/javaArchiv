...
public class Test  {
    ...
    public static void shouldCalculate() {
        logMethod();
        long number1= 12, number2= 35, number3= 117;
    ...
    public static void shouldShutdown() {
        ...
        long[] running= engine.getRunningJobs();
        System.out.println("running = " + Arrays.toString(running) +
                " (should be ["+number3+","+number4+"] in arbitrary order)");
        ...
        System.out.println("runningAfterShutdown.length = " +
                runningAfterShutdown.length + " (should be 0)");

        long[] observed1AfterShutdown= engine.getFactors(number1);
        System.out.println("observed1AfterShutdown.length = "+
                observed1AfterShutdown.length+" (should be > 0)");
        long[] observed2AfterShutdown= engine.getFactors(number2);
        System.out.println("observed2AfterShutdown.length = "+
                observed2AfterShutdown.length+" (should be > 0)");
    }
    ...
    private static long[] runAndWait1sec(final FactorsEngine engine, final long number) {
        ...
        System.out.println("progress1 = "+progress1+", progress2 = "
                +progress2+" (should read progress1 <= progress2 = 1.0)");
