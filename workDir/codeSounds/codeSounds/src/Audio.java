...
public class Audio {
    public static String driverName; //"MASTER" für default sound card
    ...
    public static void setMasterOutputVolume(float value) {
        ...
        Line line = getMasterOutputLine(driverName);
    ...
    //"PCM" um thunderbold interface anzusteuern
    //"Master" für default audio driver
    public static Line getMasterOutputLine(String s) {
        for (Mixer mixer : getMixers()) {
            for (Line line : getAvailableOutputLines(mixer)) {
                if (line.getLineInfo().toString().contains(s))
                    return line;
            }
        }
        return null;
    ...
    public static Line getMasterOutputLine() {
        for (Mixer mixer : getMixers()) {
            for (Line line : getAvailableOutputLines(mixer)) {
                if (line.getLineInfo().toString().contains(driverName))
                    return line;
            }
        }
        return null;
    ...
    protected static void adjustPan (Line line, float pan)
    {
        try {
            ...
            System.out.println("Cannot set pan on line: " + e);
        }
    }
