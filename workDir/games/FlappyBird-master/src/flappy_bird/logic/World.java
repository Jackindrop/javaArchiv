...
public class World
{
	...
	private final double SCROLLING_SPEED = 125;
	...
	public void gameTick()
	{
		...
		if (xOffset < -GameStats.PIPE_WIDTH)
		{
			pipes.remove(0);
			pipes.add(getRandomPipeHeight());
	...
	public boolean isPlayerDead()
	{
		...
		HitBox playerBox = new HitBox(GameStats.PLAYER_OFFSET, player.getHeight(), GameStats.PLAYER_WIDTH, GameStats.PLAYER_HEIGHT);
		...
		for (int i = 0; i < pipes.size(); i++)
		{
			...
			pipeBoxes.add(new HitBox(xOffset + i * GameStats.PIPE_INTERVAL, pipes.get(i) + GameStats.PIPE_SEPARATION, GameStats.PLAYER_WIDTH, screenHeight - pipes.get(i) - GameStats.PIPE_SEPARATION));
		}
